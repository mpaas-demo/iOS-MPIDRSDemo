...
/**
 mixed audio buffer for remote track.
 callback not in main thread.
 
 attention, data should be released after using it.
 */
-(void)didOutputRemoteMixedAudioBuffer:(ARTVCAudioData*)audioData;
//-(void)didOutputAudioVolume:(double)volume;
/**
 
 */
- (void)didCustomAudioDataNeeded;
/**
 通话过程中当前采集设备采集到音频音量发生变化，连续8s内最多回调一次音量低事件
 isLow为YES代表分贝较低(业务可做当前采集设备异常，请更换采集设备，如插上耳机等);isLow为NO代表恢复正常
 duration:低音量持续时长，单位ms
 maxDecibel:低音量期间最大分贝数
 */
-(void)didSoundVolumeBecomeLow:(BOOL)isLow duration:(int)duration maxDecibel:(double)maxDecibel;

/**
 The result of response of custom signaling which is sent to Room server.
 The params see in https://yuque.antfin-inc.com/rtsm/artvcroom/gprbhy#ZkpHA
 */
...
@property(nonatomic,copy) NSString* _Nullable subbiz;
...
/*
 need to callback remote mixed audio buffer
 default is NO
 */
@property(nonatomic,assign) BOOL enableRemoteMixedAudioBufferOutput;
@property(nonatomic,assign) BOOL enableAudioVolumeOutput;
...
#endif
/**
 set degradation preference for all video track excepted screen share, default is ARTVCDegradationPreferenceMAINTAIN_RESOLUTION
 suggest use [setVideoContentHint: forSourceType:] instead of this method. content hint not noly  set degradation preference but alse make a good use of video encoder
 */
...
#pragma mark - Content Hint
/**
 set content hint for specific video track. content hint can help engine to make a better  decision to fit video content,such as encoder and degradation preference.
 */
- (void)setVideoContentHint:(ARTVCVideoTrackContentHint)contentHint forSourceType:(ARTVCVideoSourceType)sourceType;
...
#pragma mark - Custom Signaling
/**
 Send custom signaling to Room server.
 See opcmd and customSignaling in https://yuque.antfin-inc.com/rtsm/artvcroom/gprbhy#ZkpHA
 
 Return requestId, you can correspond response and request by this param.
 */
-(long)sendCustomSignaling:(NSDictionary *)customSignaling opcmd:(int)opcmd;

#pragma mark - custom audio
/**
 
 */
- (void)sendCustomAudioData:(NSData *)audioData;

...
typedef NS_ENUM(int,ARTVCAudioSourceType){
    //builtin mic
    ARTVCAudioSourceType_Mic             = 0,
    //use custom audio source
    ARTVCAudioSourceType_Custom          = 1,
    //means no audio
    ARTVCAudioSourceType_Null            = 99,
    //undefined audio source type,it's an error type.used in SDK only,API Caller cann't set value to this.
    ARTVCAudioSourceType_Undefined_Error_UsedInternally = 100
};
...
//    ARTVCVideoProfileType_1920x1080_30Fps = 10,
    ...
    /**
     encoded video resolution maintain aspect ratio with screen and Largest side is1280,fps is 15
     */
    ARTVCVideoProfileType_ScreenRatio_1280_15Fps   = 14,
    /**
     encoded video resolution maintain aspect ratio with screen and Largest side is1280,fps is 30
     */
    ARTVCVideoProfileType_ScreenRatio_1280_30Fps   = 15,
    /**
...
typedef NS_ENUM(int,ARTVCErrorCode){
    ...
    */
    ...
    /**
     audio session did interrupted
     */
    ARTVCErrorCodeDidAudioSessionInterrupted              = -1020,
};
...
typedef NS_ENUM(int,ARTVCFeedType){
    ...
    //custom audio only
    ARTVCFeedTypeLocalFeedCustomAuidoOnly = 5,
};
/**
 degradation_preference options
 */
typedef NS_ENUM (int,ARTVCDegradationPreference) {
    ARTVCDegradationPreferenceDISABLED,
    ARTVCDegradationPreferenceMAINTAIN_FRAMERATE,
    ARTVCDegradationPreferenceMAINTAIN_RESOLUTION,
    ARTVCDegradationPreferenceBALANCED,
};

// See https://crbug.com/653531 and https://w3c.github.io/mst-content-hint.
/**
 * Represents content hint of video track. This exposes content hint  in C++ VideoTrackInterface class.
 */
typedef NS_ENUM(NSInteger, ARTVCVideoTrackContentHint) {
    ARTVCVideoTrackContentHintNone, //No hint has been provided,
    ARTVCVideoTrackContentHintFluid, //The track should be treated as if it contains video where motion is important
    ARTVCVideoTrackContentHintDetailed, //The track should be treated as if video details are extra important.
    ARTVCVideoTrackContentHintText //The track should be treated as if video details are extra important, and that significant sharp edges and areas of consistent color can occur frequently.
};
...
typedef NS_ENUM(int,ARTVCClientEvent) {
    ...
    /**
     audio exception
     */
    ARTVCClientEvent_SoundVolumeLow                    = 501,
    
    ARTVCClientEvent_AUDIO_FIRST_FRAME                 = 502,
    ARTVCClientEvent_VIDEO_FIRST_FRAME                 = 503,
    
    /**
     stream event
     */
    ARTVCClientEvent_FeedTypeChanged                   = 601,
#ifdef MRTC_ENABLE_VIDEO_CODEC_OPTIMIZATION
    /**
     request peer to switch video codec to codec not av1
     */
    ARTVCClientEvent_RequestPeerToSwitchVideoCodec     = 602,
#endif
    
    /**
     statictics
     */
    ARTVCClientEvent_VIDEO_CATON                       = 701,
    ARTVCClientEvent_AUDIO_CATON                       = 702,
    
    ARTVCClientEvent_SignalingCostTime                 = 703,
};
#ifdef MRTC_ENABLE_UNIFIED_PLAN
/** Represents the chosen SDP semantics for the XRTCPeerConnection. */
typedef NS_ENUM(NSInteger, MRTCSdpSemantics) {
  MRTCSdpSemanticsPlanB = 0,
  MRTCSdpSemanticsUnifiedPlan,
};
#endif
...
extern NSString *const kSoundVolumeLow;
extern NSString *const kAUDIO_FIRST_FRAME;
extern NSString *const kVIDEO_FIRST_FRAME;
extern NSString *const kFeedTypeChanged;
extern NSString *const kVIDEO_CATON;
extern NSString *const kAUDIO_CATON;
extern NSString *const kSignalingCostTime;
RTC_OBJC_EXPORT
...
/**
 additional infomation of the feed, it means media type of the feed, it's optional
 */
@property(nonatomic,readonly) NSString *mediaSource;
@property(nonatomic,assign) ARTVCFeedType feedType;
-(instancetype)initWithMediaSource:(NSString*)mediaSource;
...
typedef NS_ENUM(int,ARTVCAudioType){
    ARTVCAudioTypePCM16 = 0  //PCM 16
};
...
/**
 ...
 该方法需要在加入房间后调用。
 数据帧时长建议匹配 10ms 周期。
 该方法在音频输入设备关闭后不再生效。例如关闭本地音频、通话结束等情况下，该设置不再生效。
 */
RTC_OBJC_EXPORT
@interface ARTVCAudioFrameFormat : NSObject
@property (nonatomic, assign) ARTVCAudioType audioType;     //音频类型。当前仅支持ARTVCAudioTypePCM16。
@property (nonatomic, assign) int channels;            //音频声道数。 1：单声道。 2：双声道。当前仅支持1。
@property (nonatomic, assign) int sampleRate;          //音频采样率。默认值为48000。
@property (nonatomic, assign) int bytesPerSample;      //每个采样点的字节数。当前仅支持2。
@property (nonatomic, assign) int samplesPerChannel;   //每个声道的采样点数。sampleRate / (1000 / 10); (假设采样周期是10ms)。
@end
